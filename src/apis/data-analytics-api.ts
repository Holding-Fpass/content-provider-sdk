...
export const DataAnalyticsApiAxiosParamCreator = function (configuration?: Configuration) {
    return {
         ...
         * @throws {RequiredError}
         */
        legacyGetWatchedContent: async (options: any = {}): Promise<RequestArgs> => {
            const localVarPath = `/report/legacy/user-watched-content`;
            // use dummy base URL string because the URL constructor only accepts absolute URLs.
            const localVarUrlObj = new URL(localVarPath, 'https://example.com');
            let baseOptions;
            if (configuration) {
...
export const DataAnalyticsApiFp = function(configuration?: Configuration) {
    return {
         ...
         * @throws {RequiredError}
         */
        async legacyGetWatchedContent(options?: any): Promise<(axios?: AxiosInstance, basePath?: string) => AxiosPromise<void>> {
            const localVarAxiosArgs = await DataAnalyticsApiAxiosParamCreator(configuration).legacyGetWatchedContent(options);
            return (axios: AxiosInstance = globalAxios, basePath: string = BASE_PATH) => {
                const axiosRequestArgs = {...localVarAxiosArgs.options, url: basePath + localVarAxiosArgs.url};
                return axios.request(axiosRequestArgs);
            };
...
export const DataAnalyticsApiFactory = function (configuration?: Configuration, basePath?: string, axios?: AxiosInstance) {
    return {
         ...
         * @throws {RequiredError}
         */
        legacyGetWatchedContent(options?: any): AxiosPromise<void> {
            return DataAnalyticsApiFp(configuration).legacyGetWatchedContent(options).then((request) => request(axios, basePath));
        },
    };
};
...
export class DataAnalyticsApi extends BaseAPI {
     ...
     * @memberof DataAnalyticsApi
     */
    public legacyGetWatchedContent(options?: any) {
        return DataAnalyticsApiFp(this.configuration).legacyGetWatchedContent(options).then((request) => request(this.axios, this.basePath));
    }
}
